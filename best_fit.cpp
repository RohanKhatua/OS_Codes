...
int main()
{
    ...
    // For each process we try to accomodate it in the block which minimizes internal fragmentation
    ...
    for (int i = 0; i < num_processes; i++)
    {
        ...
        for (int j = 0; j < num_segments; j++)
        {
            ...
            if (diff < 0 || segments[j].allocated)
            {
                ...
                wasted_spaces.push_back(INT_MAX);
            }
            else
            {
        ...
        int minimum_wasted_space = *min_element(wasted_spaces.begin(), wasted_spaces.end());
        if (minimum_wasted_space != INT_MAX)
        { // Check if a valid segment was found
            // Find the segment index at which there is least wasted space
            int segment_index = find(wasted_spaces.begin(), wasted_spaces.end(), minimum_wasted_space) - wasted_spaces.begin();
